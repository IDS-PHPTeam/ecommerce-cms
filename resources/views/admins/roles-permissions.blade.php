...
@section('content')
<div class="card">
    <h2 style="margin-bottom: 1.5rem; font-size: 1.875rem; font-weight: 700;">Roles and Permissions</h2>
    ...
    <div id="roles-content" class="tab-content" style="display: block;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h3 style="font-size: 1.5rem; font-weight: 600;">Roles</h3>
            <button onclick="showRoleForm()" class="btn btn-primary">+ Add New</button>
        </div>

        <!-- Add/Edit Role Form -->
        <div id="role-form-container" style="display: none; margin-bottom: 2rem; padding: 1.5rem; background-color: #f9fafb; border-radius: 0.5rem;">
            <form id="role-form" method="POST" action="{{ route('roles-permissions.storeRole') }}">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 1rem;">
                    <div class="form-group">
                        <label for="role-name" class="form-label">Role Name <span style="color: #ef4444;">*</span></label>
                        <input type="text" id="role-name" name="name" required class="form-input" placeholder="e.g., Super Admin">
                    </div>
                    <div class="form-group">
                        <label for="role-slug" class="form-label">Slug <span style="color: #ef4444;">*</span></label>
                        <input type="text" id="role-slug" name="slug" required class="form-input" placeholder="e.g., super-admin">
                    </div>
                </div>
                <div class="form-group">
                    <label for="role-description" class="form-label">Description</label>
                    <textarea id="role-description" name="description" rows="3" class="form-input" placeholder="Role description"></textarea>
                </div>
                <div style="display: flex; gap: 1rem;">
                    <button type="submit" class="btn btn-primary">Save</button>
                    <button type="button" onclick="hideRoleForm()" class="btn" style="background-color: #6b7280; color: white;">Cancel</button>
                </div>
            </form>
        </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background-color: #f3f4f6; border-bottom: 2px solid #e5e7eb;">
                        ...
                        <th style="padding: 0.75rem; text-align: left; font-weight: 600;">Slug</th>
                        <th style="padding: 0.75rem; text-align: left; font-weight: 600;">Description</th>
                        <th style="padding: 0.75rem; text-align: left; font-weight: 600;">Permissions</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #e5e7eb;">
                        ...
                        <td style="padding: 0.75rem;">
                            <div style="display: flex; gap: 0.5rem;">
                                <button onclick="editRole({{ $role->id }}, '{{ addslashes($role->name) }}', '{{ addslashes($role->slug) }}', '{{ addslashes($role->description ?? '') }}')" class="action-btn action-btn-edit" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <form action="{{ route('roles-permissions.destroyRole', $role) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this role?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="5" style="padding: 2rem; text-align: center; color: #6b7280;">No roles found. Create your first role!</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="permissions-content" class="tab-content" style="display: none;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h3 style="font-size: 1.5rem; font-weight: 600;">Permissions</h3>
            <button onclick="showPermissionForm()" class="btn btn-primary">+ Add New</button>
        </div>

        <!-- Add/Edit Permission Form -->
        <div id="permission-form-container" style="display: none; margin-bottom: 2rem; padding: 1.5rem; background-color: #f9fafb; border-radius: 0.5rem;">
            <form id="permission-form" method="POST" action="{{ route('roles-permissions.storePermission') }}">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 1rem;">
                    <div class="form-group">
                        <label for="permission-name" class="form-label">Permission Name <span style="color: #ef4444;">*</span></label>
                        <input type="text" id="permission-name" name="name" required class="form-input" placeholder="e.g., Create Products">
                    </div>
                    <div class="form-group">
                        <label for="permission-slug" class="form-label">Slug <span style="color: #ef4444;">*</span></label>
                        <input type="text" id="permission-slug" name="slug" required class="form-input" placeholder="e.g., create-products">
                    </div>
                    <div class="form-group">
                        <label for="permission-group" class="form-label">Group</label>
                        <input type="text" id="permission-group" name="group" class="form-input" placeholder="e.g., Products">
                    </div>
                </div>
                <div class="form-group">
                    <label for="permission-description" class="form-label">Description</label>
                    <textarea id="permission-description" name="description" rows="3" class="form-input" placeholder="Permission description"></textarea>
                </div>
                <div style="display: flex; gap: 1rem;">
                    <button type="submit" class="btn btn-primary">Save</button>
                    <button type="button" onclick="hidePermissionForm()" class="btn" style="background-color: #6b7280; color: white;">Cancel</button>
                </div>
            </form>
        </div>
        ...
        <div style="margin-bottom: 2rem;">
            <h4 style="font-size: 1.25rem; font-weight: 600; margin-bottom: 1rem; color: #374151;">{{ $group ?? 'Other' }}</h4>
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background-color: #f3f4f6; border-bottom: 2px solid #e5e7eb;">
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom: 1px solid #e5e7eb;">
                            <td style="padding: 0.75rem; font-weight: 500;">{{ $permission->name }}</td>
                            ...
                            <td style="padding: 0.75rem;">
                                <div style="display: flex; gap: 0.5rem;">
                                    <button onclick="editPermission({{ $permission->id }}, '{{ $permission->name }}', '{{ $permission->slug }}', '{{ $permission->description ?? '' }}', '{{ $permission->group ?? '' }}')" class="action-btn action-btn-edit" title="Edit">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </button>
                                    <form action="{{ route('roles-permissions.destroyPermission', $permission) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this permission?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="action-btn action-btn-delete" title="Delete">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
    ...
    <div id="assign-content" class="tab-content" style="display: none;">
        <h3 style="font-size: 1.5rem; font-weight: 600; margin-bottom: 1.5rem;">Assign Permissions to Roles</h3>
        ...
        <div style="margin-bottom: 2rem; padding: 1.5rem; background-color: #f9fafb; border-radius: 0.5rem;">
            <form method="POST" action="{{ route('roles-permissions.updateRolePermissions', $role) }}">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h4 style="font-size: 1.125rem; font-weight: 600; color: #374151;">{{ $role->name }}</h4>
                    <div style="display: flex; gap: 0.5rem; align-items: center;">
                        ...
                        <button type="submit" class="btn btn-primary" style="padding: 0.5rem 1rem; font-size: 0.875rem;">Save</button>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 0.75rem;" id="permissions-container-{{ $role->id }}">
                    ...
                    <label style="display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem; background-color: white; border-radius: 0.25rem; cursor: pointer;">
                        ...
                        <span style="font-size: 0.875rem;">{{ $permission->name }}</span>
                    </label>
                    @endforeach
                </div>
            </form>
