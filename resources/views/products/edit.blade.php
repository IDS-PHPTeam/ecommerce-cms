    ...
    <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
            <div class="form-group">
                ...
                <input type="number" id="price" name="price" value="{{ old('price', $product->price) }}" step="0.01" min="0" required class="form-input" placeholder="0.00">
                @error('price')
                    <span class="form-error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="category" name="category" class="form-input">
                    ...
                    @foreach($categories as $categoryName)
                        <option value="{{ $categoryName }}" {{ old('category', $product->category) == $categoryName ? 'selected' : '' }}>{{ $categoryName }}</option>
                    @endforeach
                </select>
                @error('category')
                    <span class="form-error">{{ $message }}</span>
