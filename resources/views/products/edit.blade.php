    ...
    <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" id="productForm">
                    ...
                    <div class="multiselect-dropdown multiselect-dropdown-full" id="categoriesDropdown">
                        @foreach($categories as $category)
                            <label class="multiselect-option multiselect-option-item">
                                <input type="checkbox" name="categories[]" value="{{ $category->id }}" class="category-checkbox checkbox-custom" data-category-name="{{ $category->name }}" {{ in_array($category->id, old('categories', $product->categories->pluck('id')->toArray())) ? 'checked' : '' }}>
        ...
        <div class="form-group">
            ...
            @if($product->featured_image)
                <div id="currentImageContainer" class="mb-2 relative d-inline-block">
                    <div class="relative d-inline-block">
                        <img id="currentImage" src="{{ asset('storage/' . $product->featured_image) }}" alt="{{ $product->name }}" class="w-150 h-150 object-cover rounded border border-gray-200 d-block">
                        <button type="button" id="removeCurrentImageBtn" class="absolute remove-image-btn" title="Delete current image">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            </svg>
                        </button>
                    </div>
                    <p class="text-tertiary text-sm mt-2">Current image</p>
            ...
            @error('featured_image')
                <span class="form-error">{{ $message }}</span>
            @enderror
            <div id="imagePreview" class="mt-4 d-none relative">
                <p class="text-tertiary text-sm mb-2">New image preview:</p>
                <div class="relative d-inline-block">
                    <img id="previewImg" src="" alt="Preview" class="preview-image-large">
                    <button type="button" id="removeImageBtn" class="absolute remove-image-btn" title="Remove image">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                    </button>
        ...
        <div class="form-group">
            ...
            <div id="galleryContainer" class="mt-4 grid grid-auto-150 gap-4">
                @foreach($product->gallery as $index => $galleryItem)
                    <div class="gallery-item relative border border-gray-200 rounded-md overflow-hidden bg-white" data-gallery-id="{{ $galleryItem->id }}">
                        ...
                        @if($galleryItem->media_type === 'video')
                            <div class="relative aspect-square bg-black">
                                <video src="{{ asset('storage/' . $galleryItem->media_path) }}" class="absolute top-0 left-0 w-full h-full object-contain"></video>
                                <button type="button" class="remove-gallery-item-btn absolute remove-image-btn" title="Remove from gallery">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
                        ...
                        @else
                            <div class="relative aspect-square bg-gray-100">
                                <img src="{{ asset('storage/' . $galleryItem->media_path) }}" alt="Gallery item" class="absolute top-0 left-0 w-full h-full object-cover">
                                <button type="button" class="remove-gallery-item-btn absolute remove-image-btn" data-gallery-id="{{ $galleryItem->id }}" title="Remove from gallery">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
        ...
        <div id="simpleFieldsTab" class="tab-content d-none">
            ...
            <h3 class="section-heading my-6">Stock Management</h3>
            <div class="form-group">
            ...
            <div id="stockQuantityField" class="{{ old('track_stock', $product->track_stock ? '1' : '0') == '1' ? '' : 'd-none' }}">
                <div class="form-group">
            ...
            <div id="stockStatusField" class="{{ old('track_stock', $product->track_stock ? '1' : '0') == '0' ? '' : 'd-none' }}">
                <div class="form-group">
        ...
        <div id="attributesTabContent" class="tab-content {{ old('product_type', $product->product_type) == 'variable' ? '' : 'd-none' }}">
            <div class="form-group">
                ...
                <div class="grid grid-auto-200 gap-4">
                    @foreach($attributes as $attribute)
                        ...
                        @endphp
                        <label class="flex items-center p-3 border border-gray-300 rounded cursor-pointer transition-all variant-attribute-label {{ $isUsed ? 'border-primary-blue bg-blue-50' : '' }}">
                            <input type="checkbox" class="variant-attribute-checkbox checkbox-custom-sm" value="{{ $attribute->id }}" data-attribute-name="{{ $attribute->name }}" data-attribute-values="{{ json_encode($attribute->values->pluck('value', 'id')->toArray()) }}" {{ $isUsed ? 'checked' : '' }}>
                            <div class="flex-1">
        ...
        <div id="variationsTabContent" class="tab-content {{ old('product_type', $product->product_type) == 'variable' ? '' : 'd-none' }}">
            <div id="variantsList">
                @foreach($product->variants as $index => $variant)
                    <div class="variant-form border border-gray-200 rounded-md p-6 mb-4 bg-gray-50" data-variant-id="{{ $variant->id }}">
                        <div class="flex justify-between items-center mb-4 pb-3 border-b-2 border-primary-blue">
                            <h5 class="text-base font-semibold text-secondary m-0">Variant {{ $index + 1 }}</h5>
                            <button type="button" class="remove-variant-btn btn-danger-sm">Remove</button>
                        </div>
                        ...
                        <div class="form-group mb-6">
                            ...
                            <div class="attributes-repeater mt-2">
                                @foreach($variant->attributes as $attrIndex => $attribute)
                                    ...
                                    @endphp
                                    <div class="attribute-row flex gap-2 items-end mb-2">
                                        <div class="form-group flex-1">
                                            <label class="form-label">Attribute</label>
                                            <select class="form-input attribute-select w-full" name="variants[{{ $index }}][attributes][{{ $attrIndex }}][attribute_id]">
                                        ...
                                        <div class="form-group flex-1">
                                            ...
                                            <select class="form-input attribute-value-select w-full" name="variants[{{ $index }}][attributes][{{ $attrIndex }}][value_id]">
                                        ...
                                        <button type="button" class="remove-attribute-row-btn btn-danger-sm h-fit mb-2">Remove</button>
                                    </div>
                                @endforeach
                            </div>
                            <button type="button" class="add-attribute-row-btn btn bg-gray-600 text-white text-sm mt-2" data-variant-index="{{ $index }}">+ Add Attribute</button>
                        </div>
                        ...
                        <div class="stock-quantity-field mb-4 {{ old("variants.{$index}.track_stock", $variant->track_stock ? '1' : '0') == '1' ? '' : 'd-none' }}">
                            <div class="form-group">
                        ...
                        <div class="stock-status-field mb-4 {{ old("variants.{$index}.track_stock", $variant->track_stock ? '1' : '0') == '0' ? '' : 'd-none' }}">
                            <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            @if($variant->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $variant->image) }}" alt="Variant image" class="w-150 h-150 rounded border border-gray-200">
                                </div>
                            @endif
        ...
        <div id="variableProductFields" class="d-none">
        </div>
...
<div id="productFormButtons" class="product-form-buttons">
    <button type="submit" form="productForm" class="btn btn-primary btn-fixed-bottom">{{ __('cms.save') }}</button>
    <a href="{{ route('products.index') }}" class="btn bg-gray-600 text-white btn-fixed-bottom">{{ __('cms.cancel') }}</a>
</div>
...
<div id="imageSelectionModal" class="modal-overlay d-none">
    <div class="modal-content modal-content-large">
        <div class="modal-header">
            <h3 class="text-xl font-bold text-secondary">Choose Media</h3>
        ...
        <div class="flex border-b border-gray-200">
            <button type="button" class="image-tab-btn active flex-1 p-4 bg-none border-0 border-b-2 border-primary-blue text-primary-blue font-semibold cursor-pointer" data-tab="media">Select from Media</button>
        ...
        <div class="modal-body modal-body-large">
            ...
            <div id="modalUploadTab" class="tab-content d-none">
                <div class="text-center p-8">
                    <input type="file" id="modalFileInput" accept="image/*,video/*" multiple class="form-input max-w-400 mx-auto">
                    <p class="text-tertiary text-sm mt-4">Select one or more image or video files from your computer</p>
                </div>
            </div>
        </div>

        <div class="modal-footer flex justify-between items-center p-4 border-t border-gray-200">
            <div id="selectedCount" class="text-tertiary text-sm font-medium">
            ...
            <div class="flex gap-3">
                ...
                <button type="button" class="btn btn-primary d-none" id="addSelectedToGallery">Add Selected (<span id="addSelectedCount">0</span>)</button>
            </div>
        </div>
    </div>
</div>
