    ...
    <form method="GET" action="{{ route('dashboard') }}">
        <div style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: end;">
            ...
            <div>
                ...
                <a href="{{ route('dashboard') }}" class="btn" style="background-color: #6b7280; color: white; margin-left: 0.5rem;">{{ __('cms.reset') }}</a>
            </div>
        </div>
    </form>
</div>
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem; margin-bottom: 1.5rem;">
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.total_orders') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #1f2937;">{{ number_format($totalOrders) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.successful') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #059669;">{{ number_format($successfulOrders) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.canceled') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #dc2626;">{{ number_format($canceledOrders) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.failed') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #dc2626;">{{ number_format($failedOrders) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.total_value') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #1f2937;">${{ number_format($totalValue, 2) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.total_commission') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #1f2937;">${{ number_format($totalCommission, 2) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.current_orders_assigned') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #2563eb;">{{ number_format($assignedOrders) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.current_orders_not_assigned') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #f59e0b;">{{ number_format($notAssignedOrders) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.active_drivers_total_drivers') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #1f2937;">{{ number_format($activeDrivers) }} / {{ number_format($totalDrivers) }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.average_delivery_time') }}</div>
        <div style="font-size: 2rem; font-weight: 700; color: #1f2937;">{{ $averageDeliveryTime }} {{ __('cms.min') }}</div>
    </div>
    <div class="card" style="padding: 1.5rem;">
        <div style="font-size: 0.875rem; color: #6b7280; margin-bottom: 0.5rem;">{{ __('cms.failed_delivery_rate') }}</div>
...
<div class="card" style="margin-bottom: 1.5rem;">
    <div style="padding: 1rem; border-bottom: 1px solid #e5e7eb;">
        <h3 style="font-size: 1.25rem; font-weight: 600;">{{ __('cms.live_map') }}</h3>
    </div>
    <div style="padding: 1rem;">
        <div id="map" style="width: 100%; height: 500px; background-color: #f3f4f6; border-radius: 0.5rem; display: flex; align-items: center; justify-content: center; color: #6b7280;">
            <div style="text-align: center;">
                ...
                <p style="font-size: 0.875rem; margin-top: 0.5rem;">{{ __('cms.driver_locations') }}: {{ $mapDrivers->count() }}</p>
                <p style="font-size: 0.875rem;">{{ __('cms.order_locations') }}: {{ $mapOrders->count() }}</p>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 2rem; margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #e5e7eb;">
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                ...
                <span style="font-size: 0.875rem;">{{ __('cms.driver_locations') }} ({{ $mapDrivers->count() }})</span>
            </div>
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <div style="width: 16px; height: 16px; background-color: #10b981; border-radius: 50%;"></div>
                <span style="font-size: 0.875rem;">{{ __('cms.assigned_orders') }} ({{ $mapOrders->where('status', 'assigned')->count() }})</span>
            </div>
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <div style="width: 16px; height: 16px; background-color: #f59e0b; border-radius: 50%;"></div>
                <span style="font-size: 0.875rem;">{{ __('cms.unassigned_orders') }} ({{ $mapOrders->where('status', 'pending')->count() }})</span>
            </div>
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <div style="width: 16px; height: 16px; background-color: #ef4444; border-radius: 50%;"></div>
                <span style="font-size: 0.875rem;">{{ __('cms.failed_delays') }} ({{ $mapOrders->where('status', 'failed')->count() }})</span>
            </div>
        </div>
    </div>
</div>
