...
@section('content')
<div class="card">
    <h2 style="margin-bottom: 1.5rem; font-size: 1.875rem; font-weight: 700;">Edit Attribute</h2>
    ...
    <form method="POST" action="{{ route('attributes.update', $attribute) }}">
        ...
        <div class="form-group">
            ...
            <input type="text" id="name" name="name" value="{{ old('name', $attribute->name) }}" required class="form-input" placeholder="Attribute name">
            @error('name')
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" rows="3" class="form-input" placeholder="Attribute description">{{ old('description', $attribute->description) }}</textarea>
            @error('description')
                <span class="form-error">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <div id="valuesContainer" style="margin-bottom: 0.5rem;">
                @foreach($attribute->values->sortBy('sort_order') as $index => $value)
                    <div class="value-item" data-value-id="{{ $value->id }}" style="display: flex; gap: 0.5rem; margin-bottom: 0.5rem; align-items: center; padding: 0.5rem; background-color: #f9fafb; border-radius: 0.375rem; cursor: move;">
                        ...
                        <input type="text" name="values[]" value="{{ old("values.{$index}", $value->value) }}" class="form-input" placeholder="Enter value" style="flex: 1;">
                        <button type="button" class="remove-value-btn" style="background-color: #ef4444; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.25rem; cursor: pointer; {{ $attribute->values->count() == 1 ? 'display: none;' : '' }}">Remove</button>
                    </div>
                @endforeach
                @if($attribute->values->isEmpty())
                    <div class="value-item" style="display: flex; gap: 0.5rem; margin-bottom: 0.5rem; align-items: center; padding: 0.5rem; background-color: #f9fafb; border-radius: 0.375rem; cursor: move;">
                        ...
                        <input type="text" name="values[0]" class="form-input" placeholder="Enter value" style="flex: 1;">
